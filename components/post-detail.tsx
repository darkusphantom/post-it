...
export default async function PostDetail({ slug }: { slug: string }) {
  try {
    ...
    if (!post) {
      return (
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground mt-2">La publicación que estás buscando no existe o ha sido eliminada.</p>
        </div>
      )
    }
    ...
    return (
      <div className="text-center py-12">
        <h2 className="text-xl font-semibold">Error al cargar la publicación</h2>
        <p className="text-muted-foreground mt-2">Hubo un problema al cargar la publicación. Por favor, intenta nuevamente.</p>
      </div>
    )
  }
}
